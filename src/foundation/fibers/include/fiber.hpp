...
#include "foundation/fibers/src/fcontext.hpp"
...
namespace opus3d::foundation
{
	...
	public:
		...
		FiberContext(void* stackBase, size_t stackSize, FiberContextFunc entry, void* userArg) : m_func(entry), m_userArg(userArg)
		{
			...
			m_context = boost::context::make_fcontext(reinterpret_cast<void*>(sp), alignedSize, &FiberContext::entry_stub);
		}
	...
	public:
		...
		Fiber(void* stack, size_t size, Task task) : m_task(static_cast<Task&&>(task)), m_ctx(stack, size, &Fiber::run_task, this) { m_ctx.start(); }
